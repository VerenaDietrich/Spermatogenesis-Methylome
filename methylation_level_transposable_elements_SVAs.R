...
## Define the regions of interest given in the repeat-maske - Modify according to your needs
types <- c("SVA_A", "SVA_B", "SVA_C", "SVA_D", "SVA_E", "SVA_F") 
...
##  Exctract methylation levels of interest
...
#write.csv(df_statsitc, "/path/to/results/statistics_retroposons/result_repeatMasker_transposons_SVAs_minCov_statistic.csv", row.names=FALSE)

# Histogramm
png(file="/path/to/results/statistics_retroposons/transposons_SVAs_overlap_hist.png",
...
ggsave("/path/to/results/statistics_retroposons/transposons_SVAs_min_cov_boxplot.png", plot = mincover_plot, width = 7.5, height = 6, dpi = 300)
...
mincover_plot_jitter <- ggplot(data = df_statsitc, aes(x = type, y = overlap_minCov_ratio, color = type)) +
  ...
  labs(title="Ratio fullfilling min coverage of 5", x="Retrotransposons", y="Overlap ratio")
ggsave("/path/to/results/statistics_retroposons/transposons_SVAs_min_cov_jitterplot.png", plot = mincover_plot_jitter, width = 7.5, height = 6, dpi = 300)
...
#write.csv(methylation_combined, "/path/to/results/result_repeatMasker_transposons_SVA_s_minCov.csv", row.names=FALSE)
#methylation_combined <- read.csv('/path/to/results/result_repeatMasker_transposons_SVA_s_minCov.csv', header=TRUE)
...
methylation_combined$Type <- factor(methylation_combined$Type, levels = c("SVA_A", "SVA_B", "SVA_C", "SVA_D", "SVA_E", "SVA_F"))

################################## Distribution of SVA_D in den einzelnen Patienten


### Violine plot
methylation_combined_splitplot <- methylation_combined[methylation_combined$State %in% c("Undiff", "Diff", "4N"),]
methylation_combined_splitplot$Sample <- as.character(methylation_combined_splitplot$Sample)

g1 <- ggplot(data = methylation_combined_splitplot, mapping = aes(y=Methylation, x=Sample, fill = State)) + 
...
#ggsave("/path/to/results/retransposons_samples_seperated_SVA_s.png", plot = g1, width = 15, height = 8.7, units = "cm", dpi = 300)

### Split violine plot (Publication)
...
#ggsave("/path/to/results/Repeat_retroposons_SVAs_Splitplot_minCov_white_linewidth_area.png", plot = plot_1, width = 8.7, height = 8.7, units = "cm", dpi = 300)
